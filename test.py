...
def runTest(image : Image, scale : int = 2):
	...
	handler = upscale.Upscaler('result.pth', scale)
	upscaledImage = handler.upscaleImage(low_res_image)
	correspondance = check.compareImages(upscaledImage, high_res_image)
	print("After downscaling and upscaling by a factor ", scale, ", the accuracy on this image is:", sep='')
...
if __name__ == "__main__":
	...
	runTest(Image.open(args.imagePath))